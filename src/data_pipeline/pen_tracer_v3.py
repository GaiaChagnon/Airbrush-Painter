"""Gamut-aware pen layer extraction (V3).
...
def make_pen_layer_v3(
    ...
    if cmy_canvas_path and Path(cmy_canvas_path).exists():
        cmy_img = cv2.imread(cmy_canvas_path)
        cmy_img = cv2.cvtColor(cmy_img, cv2.COLOR_BGR2RGB)
        cmy_img = cv2.resize(cmy_img, (W, H))
        composite = cmy_img.copy()
        mask = (canvas < 250).any(axis=2)

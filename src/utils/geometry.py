...
def bezier_cubic_polyline(
    ...
    max_err_px : float
        Maximum allowed deviation in pixels, default 0.25 (sub-pixel)
    max_depth : int
    ...
    Stops when curve is sufficiently flat (within max_err_px) or max_depth reached.
    
    Flatness criterion: distance from control points to chord < max_err_px.
    For mm → px conversion, assumes ~3.5 px/mm (typical for 908px / 210mm).
    Caller should pass max_err_px scaled appropriately.
    """
    ...
    def subdivide(q1, q2, q3, q4, depth):
        ...
        max_dist = max(d2.item(), d3.item())
        
        # Heuristic conversion: mm to px (assume ~3.5 px/mm for A4)
        px_per_mm = 908.0 / 210.0  # Default for render_px
        max_dist_px = max_dist * px_per_mm
        
        if max_dist_px <= max_err_px:
            # Sufficiently flat
            return torch.stack([q1, q4], dim=0)

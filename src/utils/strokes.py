...
def stroke_vec_to_yaml_dict(
    ...
    """Convert 15-D stroke vector (mm-space) to YAML dictionary.
    ...
    bounds : Dict[str, List[float]], optional
        Schema bounds for validation (not enforced here)
    ...
    Notes
    -----
    All coordinates in mm. Does NOT validate bounds (use validators.py).
    """
    ...
    return {
        'id': stroke_id,
        'bezier': {
            'p1': [float(vec_mm[0]), float(vec_mm[1])],
            'p2': [float(vec_mm[2]), float(vec_mm[3])],
            'p3': [float(vec_mm[4]), float(vec_mm[5])],
            'p4': [float(vec_mm[6]), float(vec_mm[7])]
        },
        'z_profile': {
            'z0': float(vec_mm[8]),
            'z1': float(vec_mm[9])
